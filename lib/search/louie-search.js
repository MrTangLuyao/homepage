...
(function () {
  ...
  function coreItems() {
    return [
      ...
      { g: UI.gSocial, icon: 'link', title: { zh: 'JimmyXue 的主页', en: "JimmyXue's Homepage" }, sub: 'brandyxue.com', hint: { zh: '友链', en: 'friend' }, url: 'https://brandyxue.com', newTab: true, kw: 'jimmyxue brandyxue friend 友链 朋友' }
    ];
  }
  ...
  /* ─── Remote items: blog librarians + learn broadcast + extra_content ─── */
  let remoteItems = [];
  let recentItems = [];
  ...
  /* Re-entrant: called on every open(). Returns immediately once a
     complete snapshot is in memory; otherwise retries failed sources
     with a 10 s back-off, so one bad fetch round doesn't stick. */
  async function loadRemote() {
    if (remoteComplete || remoteLoading) return;
    if (Date.now() - lastAttempt < 10000) return;
    remoteLoading = true;
    lastAttempt = Date.now();
    const items = [];

    /* v2: v1 snapshots could be partial (pre-learn or failed rounds) */
    const CK = 'louie-search-cache-v2';
    try {
      const c = JSON.parse(sessionStorage.getItem(CK));
      if (c && c.complete && Date.now() - c.t < 30 * 60 * 1000) {
        remoteItems = c.items;
        remoteComplete = true;
        remoteLoading = false;
        finishRemote();
    ...
    let blogOk = false, learnOk = false, extraOk = false;
    ...
    remoteItems = items;
    remoteComplete = blogOk && learnOk && extraOk;
    /* Never persist a partial snapshot — one failed source would
       otherwise poison every page in this tab for 30 minutes. */
    if (remoteComplete) {
      try { sessionStorage.setItem(CK, JSON.stringify({ t: Date.now(), items, complete: true })); } catch (e) {}
    }
    remoteLoading = false;
    finishRemote();
  }
  ...
  const STYLE = `
    ...
    * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    .root {
      ...
      font-size: 15px; line-height: 1.6; color: var(--text, #e8e8e8);
    }
    kbd {
      font: inherit; font-size: 11px; color: var(--muted, #a1a1a0);
      background: var(--surface-high, #333332); border: 1px solid var(--border, #3a3a39);
    ...
    .panel {
      ...
      background: var(--surface, #2a2a29);
      border: 1px solid var(--border, #3a3a39);
      border-radius: 16px;
      box-shadow: var(--search-shadow, 0 24px 64px rgba(0, 0, 0, 0.5));
      overflow: hidden;
      animation: in 0.16s ease-out;
    }
    @keyframes in { from { opacity: 0; transform: translateY(-6px); } }
    .inputrow { display: flex; align-items: center; gap: 12px; padding: 14px 16px; }
    .inputrow > svg { width: 18px; height: 18px; color: var(--muted, #a1a1a0); flex-shrink: 0; }
    input {
      flex: 1; min-width: 0; font: inherit; font-size: 15px;
      color: var(--text, #e8e8e8); background: none; border: none; outline: none; padding: 0;
    }
    input::placeholder { color: var(--muted, rgba(161, 161, 160, 0.7)); }
    .keys { display: flex; gap: 5px; flex-shrink: 0; }
    .list {
      max-height: min(380px, 48vh); overflow-y: auto;
      padding: 6px;
      border-top: 1px solid var(--border, #3a3a39);
      overscroll-behavior: contain;
      scrollbar-width: thin; scrollbar-color: var(--outline, #4a4a49) transparent;
    }
    .group {
      font-size: 10.5px; font-weight: 500; letter-spacing: 0.14em;
      text-transform: uppercase; color: var(--muted, rgba(161, 161, 160, 0.75));
    ...
    .item:hover { background: var(--surface-high, #333332); }
    .item.sel { background: var(--accent-dim, rgba(217, 119, 87, 0.15)); }
    .ic { width: 18px; height: 18px; flex-shrink: 0; color: var(--muted, #a1a1a0); }
    .ic svg { width: 18px; height: 18px; display: block; }
    .item.sel .ic { color: var(--accent-on, #ffb49d); }
    .tx { flex: 1; min-width: 0; display: flex; align-items: baseline; gap: 9px; }
    .ti {
      font-size: 14px; font-weight: 600; color: var(--text, #e8e8e8);
      white-space: nowrap; flex-shrink: 0;
      max-width: 75%; overflow: hidden; text-overflow: ellipsis;
    }
    .item.sel .ti { color: var(--accent-on, #ffb49d); }
    .su {
      font-size: 12px; color: var(--muted, #a1a1a0);
      white-space: nowrap; overflow: hidden; text-overflow: ellipsis; min-width: 0;
    }
    .hi {
      font-size: 11px; color: var(--muted, rgba(161, 161, 160, 0.6));
      white-space: nowrap; flex-shrink: 0;
    }
    .empty { padding: 24px 14px; text-align: center; color: var(--muted, #a1a1a0); font-size: 13px; }
