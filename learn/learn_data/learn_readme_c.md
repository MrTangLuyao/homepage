...
- **C：~25–30 MB**（clang.wasm + libc/libc++ 归档）首次进 C 课程或 playground 时；之后免费
...
| `chapterRef`      | string         | 课时文件              | 可选——链到博客文章                              |
...
1. 挑 `id`（下一个空整数；当前 C 课程是 1–44）。
2. 挑 `slug`（`hello-c`、`pointers`、`malloc-free` 等）。
3. 决定 section：语法用 `'main'`，`<header.h>` 课时用 `'stdlib'`。
4. 建 `learn/learn_data/c/lessons/<NN>-<slug>.js`。
5. 把索引条目追加到 `course.lessons`。`manifest.js` 的 `lessonsCount` 也 +1。
