...
- `blog_data/manifest.js` — the **index** only. Metadata for every post (slug, date, title, excerpt, tags, etc.). No bodies. Loaded once at page load.
- `blog_data/<slug>/post.zh.js` and `blog_data/<slug>/post.en.js` — each post's **body**, in its own folder. Loaded on demand when the reader view opens that post.
...
`blog_data/manifest.js` sets `window.__BLOG_MANIFEST` to:
  ...
  posts: [
    {
      ...
      cover: 'cover.webp',       // OPTIONAL. Resolved as `blog_data/<slug>/cover.webp`.
...
**No `body` field.** Bodies live in `blog_data/<slug>/post.<lang>.js`.
...
2. **Create the folder**: `blog_data/<slug>/`
...
4. **(Optional) Drop a cover image** at `blog_data/<slug>/cover.webp`.

5. **Append a manifest entry** to `posts[]` in `blog_data/manifest.js`. Bump the top-level `updated`.
...
| `<img src="blog_data/<slug>/img.webp">` | Block image with rounded corners and border |
...
<img src="blog_data/<slug>/img.webp">
```

Same rule for the `cover` field in the manifest — it's the *filename* (e.g. `"cover.webp"`); the renderer prepends `blog_data/<slug>/`.
...
1. Edit `blog_data/<slug>/post.zh.js` and/or `post.en.js`. The body string is the only thing inside.
...
|---|---|
| `<script src="blog_data/manifest.js">` | Loads `window.__BLOG_MANIFEST` synchronously before the main script runs |
...
| `loadPostBody(slug, lang)` | On demand: injects a `<script>` tag for `blog_data/<slug>/post.<lang>.js`, awaits load, returns the registered body. Cached in `window.__BLOG_POSTS` and an in-flight map (`_bodyLoading`) to prevent duplicate loads. |
...
1. **`window.__BLOG_MANIFEST` must exist before the main script runs.** Keep `<script src="blog_data/manifest.js">` *before* the inline `<script>` in `blog.html`. The current order is correct — don't reorder.
...
- Page boots → `<script src="blog_data/manifest.js">` populates `window.__BLOG_MANIFEST` (~1.5 KB, all metadata, no bodies)
- List view renders from manifest only — fast, no network
- User clicks a post → `loadPostBody(slug, lang)` injects `<script src="blog_data/<slug>/post.<lang>.js">`, which registers `window.__BLOG_POSTS['<slug>:<lang>']`. Cached in memory after first load.
- Language toggle re-fetches via the same loader (with cache hit if already loaded)
- Cover images, inline images: standard `<img src="blog_data/<slug>/...">`. Resolved relative to `blog.html`, not the post folder.
