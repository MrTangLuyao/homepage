...
# ── Paths ──────────────────────────────────────────────────────────────
# __file__ lives at  blog_data/editor/blog_writer.py
# so two dirname calls reach  blog_data/
BLOG_DATA = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
class BlogWriter(tk.Tk):
    ...
    def _delete_post(self):
        ...
        if not messagebox.askyesno(
            ...
            'The files in blog_data/{slug}/ will NOT be deleted.',
        ):
            return
        posts = self._manifest.get('posts', [])
